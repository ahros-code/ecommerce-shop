...
import {ConfirmCodeModel, SellerModel} from "../models/index";
...
async function userRegister(req: Request, res: Response) {
  try{
    ...
    const isUserExists = await SellerModel.findOne({where: {email}});
    if(isUserExists){
      ...
      html: `
        <div>
          <h1>activation link</h1>
            <a href="${BACKEND_URL}/api/seller/auth/activate/${activateLink}">Activate account</a>
        </div>
      `
    })
...
async function activate(req, res){
  try{
    ...
    const user = await SellerModel.findOne({where: {activateLink: link}}) as any;
    if(!user){
...
async function forgotPassword(req, res){
  try{
    ...
    const user = await SellerModel.findOne({where: {email}}) as any;
    if(!user){
      return res.status(404).send({
        status: 404,
        data: null,
...
async function newPassword(req, res){
  try{
    ...
    const newUserPass = await SellerModel.update({password: hash},{where: {email}});
    return res.status(200).send({
      status: 200,
